...
$_SESSION['order'][$_GET['sid']] = $_GET['qty'];

if (!isset($_SESSION['order']) or empty($_SESSION['order'])) {
...
if (!empty($FoodKeys)) {
    ...
    $sql = sprintf("SELECT pf.*,od.* FROM `products_food` pf LEFT JOIN `order_detail` od ON pf.product_id=od.Product_Sid WHERE od.Sid='%s'", implode(",", $FoodKeys));
    // $sqlAddToCart = sprintf("SELECT pf.*,od.* FROM `products_food` pf LEFT JOIN `order_detail` od ON pf.product_id=od.Product_Sid WHERE od.Order_Sid='%s'", $FoodKeys[0]);
    // $sqlcount= sprintf("SELECT count(1) FROM `products_food` pf LEFT JOIN `order_detail` od ON pf.product_id=od.Product_Sid WHERE od.Order_Sid='%s'", $FoodKeys[0]);
    $rows = $pdo->query($sql)->fetchAll();
    foreach ($rows as $r) {
        $r['quantity'] = $_SESSION['order'][$r['Sid']];
        $FoodData[$r['Sid']] = $r;
        // echo json_encode($FoodData, JSON_UNESCAPED_UNICODE);
    ...
    <div class="row">
        <div class="col">
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    <?php
                    foreach ($_SESSION['order'] as $sid => $qty) :
                        $item = $FoodData[$sid];
                    ?>
            ...
            <?php if (isset($_SESSION['user'])) : ?>
                <a href="order-editapi.php" class="btn btn-success">送出資料</a>
            <?php else : ?>
...
<script>
    ...
    function changeQty(event) {
        ...
        console.log(sid);
        fetch('order-edit-api.php?sid=' + sid + '&qty=' + qty)
        calPrices();
    }
